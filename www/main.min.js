var accountMetadata,settings,ss,currentDWD,currentWFAACL,currentStudentID,currentAcctURL,currentAcctData,currentAcctID,TIMEOUT=3E4,TRANSITION_TIME=.325,FADE_TIME=150,ERR_FADE_TIME=200,VERSION="1.9.1",DEFAULT_SETTINGS={allowDiag:!0,allowPush:!0,pushInterval:3600,version:VERSION},IOS_BTN_FADE_TIME=200;ons.forcePlatformStyling("ios");
...
function onResume(){$("#title").text("Gradebook");$("#saveBtn").hide();var a=$("#iOSBackBtn");a.off("click.editorGoBack click.goBack").on("click.goBack",goBack);a.hide();$("#goalDialog").hide();$("#title").text("Gradebook");navi.resetToPage("mainPage",{animation:"lift",animationOptions:{duration:TRANSITION_TIME}}).then(function(){loadAcctList(currentAcctID)})}
