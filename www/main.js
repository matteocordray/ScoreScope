...
function loadAcctList(optionalAcctToLoad) {
    ...
    $.each(accountMetadata, function (key, val) { // Loads stuff in accounts list and populates hamburger menu
        menuList.append('<ons-list-item tappable modifier="longdivider" onclick="getAndParseAccount(' + key + ',loadAcct)">' + val.name + '</ons-list-item>');
    });
...
function loadAcct(data) {
    ...
    $.each(data.courses, function (key, val) { // Loads stuff in course list
        if (val.grade < 0) { // If no grades in a course
            $("#list").append('<ons-list-item tappable modifier="chevron longdivider" class="course"  onclick="alertMsg(\'There are currently no assignments in this course.\')">' +
            ...
            $("#list").append('<ons-list-item tappable modifier="chevron longdivider" class="course" ' +
