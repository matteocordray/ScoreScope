...
function getAndParseCourse(a,e,f){"undefined"===typeof f&&navi.pushPage("courses/course.html",{animation:"slide",animationOptions:{duration:TRANSITION_TIME}});var d=$.post(currentAcctURL+"gradebook003.w",{dwd:currentDWD,wfaacl:currentWFAACL,currentStudent:currentStudentID,currentExtType:a.extType,currentExtNum:a.extNum,currentGBId:a.GBID,currentCourseId:a.courseID,LinkData:"studentaccess.w"}).done(function(){var b=$((new DOMParser).parseFromString(d.responseText,"text/html"));a.categories=[];b=b.find(".categories");
if(0<b.length){for(var b=$(b[0]),f=-1,h={name:null,weight:null,average:null,items:[]};0<b.next().length;){var c;if(-1<b.attr("class").indexOf("categories")){f++;a.categories.push($.extend(!0,{},h));c=a.categories[f];c.name=$(b[0].firstChild.firstChild).text().trim();var g=$(b[0].firstChild).children().text().trim();c.weight=Math.round(+g.substr(g.indexOf("(")+1,g.indexOf("%")-1));c.average=0<b.children().eq(2).text().trim().length?+b.children().eq(2).text().trim():"&nbsp;&nbsp;";c.items=[]}else c.items.push({date:b.children().eq(0).text().trim(),
name:b.children().eq(1).text().trim(),modifier:b.children().eq(2).text().trim(),grade:b.children().eq(3).text().trim()});b=b.next()}e(a)}else $("#assignmentList").empty(),displayErrorPage("#courseErrMsgDiv","Oh No!","The server sent a malformed response.","ErrorCircle",function(){$("#loading").fadeIn(FADE_TIME);$("#courseErrMsgDiv").fadeOut(ERR_FADE_TIME,function(){getAndParseCourse(a,e,!0)})})}).fail(function(b){0===b.readyState?displayErrorPage("#courseErrMsgDiv","Oh No!","We couldn't establish a connection. Please check your internet connection and try again.",
"ErrorTriangle",function(){$("#loading").fadeIn(FADE_TIME);$("#courseErrMsgDiv").fadeOut(ERR_FADE_TIME,function(){getAndParseCourse(a,e,!0)})}):displayErrorPage("#courseErrMsgDiv","Oh No!","We couldn't retrieve assignment data (HTTP "+b.status+"). Please check your internet connection and try again.","ErrorTriangle",function(){$("#loading").fadeIn(FADE_TIME);$("#courseErrMsgDiv").fadeOut(ERR_FADE_TIME,function(){getAndParseCourse(a,e,!0)})})})}
